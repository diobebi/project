...
def evaluate_step(model, loader, metrics, device):
    ...
    for x in loader:
        for ins in x:
            with torch.no_grad():
                print(ins[4].shape)
                d = ins[4].repeat(1, 1000)
                out = model(ins[5].to(device), d.to(device))
                metrics.update(out.squeeze(),
                            x[3].to(device).squeeze(),
                            cell_lines = x[5].to(device).squeeze().to(device),
...
def train_step(model, optimizer, loader, config, device):
    ...
    for x in loader:
        ...
        # print(len(x[2][0])) # Labels 

        # print(len(x[4][0])) # drug index
        
        # print(len(x[5][0])) # cell index

        # drug_ids =  [j for i in x[4] for j in i]
        # cell_ids =  [j for i in x[5] for j in i]    
        # labels = [j for i in x[3] for j in i]
        ...
        # print(drug_ids.shape)
        # print(cell_ids.shape)

        drug_ids1 = torch.tensor(x[4][0]) 
        cell_ids1 = torch.tensor(x[5][0])
        labels1 = torch.tensor(x[2][0])

        drug_ids2 = torch.tensor(x[4][1]) 
        cell_ids2 = torch.tensor(x[5][1])
        labels2 = torch.tensor(x[2][1])

        drug_ids = torch.stack((drug_ids1, drug_ids2), 0)
        cell_ids = torch.stack((cell_ids1, cell_ids2), 0)
        labels = torch.stack((labels1, labels2), 0)

        drug_ids = drug_ids.unsqueeze(0)
        cell_ids = cell_ids.unsqueeze(0)
        labels = labels.unsqueeze(0)

        drug_ids = drug_ids.permute(0,2,1)
        cell_ids = cell_ids.permute(0,2,1)
        ...
        out = model(cell_ids.to(device), drug_ids.to(device))
        l = loss(out.squeeze(), labels.to(device).squeeze())
        l.backward()
