...
class OmicsDataset_drugwise(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cell_features = cell_features = torch.stack([self.omic_dict[cell_id].clone().detach().float() for cell_id in cell_ids])  # All cell features


        cell_features = self.pad_tensor(cell_features, 1000, 0)

        labels = Tensor(labels)  # Labels as a tensor
...
class OmicsDataset_drugwise(Dataset):          
    ...
    def __init__(self, omic_dict, drug_dict, data):
        ...
        # Group data by drug_id to enable fetching all cell lines for a drug
        self.data = data.groupby("DRUG_ID")  # Assuming drug_id is in the second column
    
    def __len__(self):
        return len(self.data)
    
    def __getitem__(self, idx):
    # Get the drug_id for the specified index
        drug_id = list(self.data.groups.keys())[idx]
        instances = self.data.get_group(drug_id)
        
        # Extract omics, targets, and cell IDs for the drug
        omics = [self.omic_dict[row.iloc[0]] for _, row in instances.iterrows()]
        targets = [row.iloc[2] for _, row in instances.iterrows()]  # Assuming target is in the third column
        cell_ids = [self.cell_mapped_ids[row.iloc[0]] for _, row in instances.iterrows()]
        
        return (self.drug_dict[drug_id],       # Drug features
                tensor(omics),                # Omics data for all cell lines
                tensor(targets),              # Target values
                tensor(cell_ids),             # Cell line indices
                tensor([self.drug_mapped_ids[drug_id]]))  # Wrap scalar in a list for a 1-element tensor
